...
<template>
  <section class="hello-world-wrapper">
    ...
    ></a-input>
      <transition-group tag="ul" :class="['options-list', {'show-list': showList}]" name="tag">
        <li v-for="(item, index) in selectOptions" :key="item.value">
          <a-checkbox v-model:checked="item.done" @click="handleOptionClick(item.label)">
...
import { ref,Ref, reactive, onMounted, computed, watch, onUnmounted, toRefs, toRef, defineComponent} from 'vue'
import {todoOperate} from './todo';
import {getStorage} from '../utils/useStorage'
import {CloseCircleOutlined} from '@ant-design/icons-vue'
// defineProps<{ msg: string }>();

const {count,showList,selectOptions,finishedCount, inputValue,selectedAll, handleKeydown,handleClearSelect,handleOptionClick, handleRemoveOption} = todoOperate();
...
.hello-world-wrapper{
  ...
  .options-list{
    ...
    li{
      ...
      &:hover{
        background-color: rgba(255,204,240,.1);
        cursor: pointer;
      }
      .done{
        color: grey;
  ...
  .tag-leave-active, .tag-enter-active{
    transition: all .5s;
  }
  .error-enter-from,
  .error-leave-to{
